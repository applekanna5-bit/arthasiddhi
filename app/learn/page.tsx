...
import { categoryDescriptions, categoryLabels, getFeaturedArticles, publishedCategories } from "@/lib/content/articles";
    ...
    <main className="flex-1 bg-slate-50 px-4 py-6 text-slate-900 sm:px-6 sm:py-8 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <section className="max-w-3xl py-12">
          ...
          <p className="mt-5 text-lg leading-8 text-slate-600">Understand how loans, SIPs, fixed deposits and compound interest work through practical explanations and worked examples.</p>
        </section>
        <section aria-labelledby="categories-heading">
          <h2 id="categories-heading" className="text-2xl font-bold tracking-tight text-slate-950">Topics</h2>
          <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {publishedCategories.map((category) => <Link key={category} href={`/learn/${category}`} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-emerald-300 hover:shadow-md focus:outline-none focus:ring-3 focus:ring-emerald-100"><h3 className="font-semibold text-slate-950">{categoryLabels[category]}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{categoryDescriptions[category]}</p><span className="mt-4 inline-block text-sm font-semibold text-emerald-700">View guides <span aria-hidden="true">→</span></span></Link>)}
          </div>
        </section>
        <section aria-labelledby="latest-heading" className="mt-12 pb-12">
          <h2 id="latest-heading" className="text-2xl font-bold tracking-tight text-slate-950">Guides worth starting with</h2>

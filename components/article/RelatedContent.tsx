...
type Calculator = { slug: CalculatorSlug; name: string; href: string; description: string };

export function RelatedCalculators({ calculators, bankingArticleSlug }: { calculators: readonly Calculator[]; bankingArticleSlug?: BankingArticleSlug }) {
  if (!calculators.length) return null;
  return <section aria-labelledby="related-calculators-heading"><h2 id="related-calculators-heading" className="text-2xl font-bold tracking-tight text-slate-950">Try a calculator</h2><div className="mt-4 grid gap-4 sm:grid-cols-2">{calculators.map((calculator) => {
    const analytics = getBankingGuideLink(bankingArticleSlug, calculator.slug, "related_calculator_card");
    return analytics ? <TrackedLink analytics={analytics} key={calculator.href} href={calculator.href} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-emerald-300 hover:shadow-md focus:outline-none focus:ring-3 focus:ring-emerald-100"><h3 className="font-semibold text-slate-950">{calculator.name}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{calculator.description}</p><span className="mt-4 inline-block text-sm font-semibold text-emerald-700">Open calculator <span aria-hidden="true">→</span></span></TrackedLink> : <Link key={calculator.href} href={calculator.href} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-emerald-300 hover:shadow-md focus:outline-none focus:ring-3 focus:ring-emerald-100"><h3 className="font-semibold text-slate-950">{calculator.name}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{calculator.description}</p><span className="mt-4 inline-block text-sm font-semibold text-emerald-700">Open calculator <span aria-hidden="true">→</span></span></Link>;
  })}</div></section>;
}
